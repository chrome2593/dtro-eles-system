  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-indigo-500/30">
      ...
      <main className="max-w-4xl mx-auto p-6 md:p-10 space-y-8">
        {/* 상단 필터 섹션 - 여백 최적화(py-4) */}
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <div className="bg-slate-900/50 border border-white/5 py-4 px-6 rounded-[2.5rem] space-y-4">
            ...
            <div className="flex gap-2 p-1 bg-slate-950 rounded-2xl">
              {Object.keys(lineData).map(line => (
                <button key={line} onClick={() => setSelection({...selection, line, station: lineData[line][0], unit: '전체'})} className={`flex-1 py-3 rounded-xl text-[11px] font-black transition-all ${selection.line === line ? 'bg-indigo-600 text-white shadow-xl' : 'text-slate-600 hover:text-slate-400'}`}>{line}</button>
              ))}
            </div>
          </div>
          <div className="bg-slate-900/50 border border-white/5 py-4 px-6 rounded-[2.5rem] space-y-4">
            <label className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.2em] block">02. Station Name</label>
            <select value={selection.station} onChange={(e) => setSelection({...selection, station: e.target.value, unit: '전체'})} className="w-full bg-slate-950 border-2 border-white/5 rounded-2xl px-4 py-3 text-sm font-black focus:border-indigo-600 outline-none text-white cursor-pointer appearance-none">{lineData[selection.line].map(s => <option key={s} value={s}>{s}역</option>)}</select>
          </div>
          <div className="bg-slate-900/50 border border-white/5 py-4 px-6 rounded-[2.5rem] space-y-4">
            <label className="text-[10px] font-black text-indigo-500 uppercase tracking-[0.2em] block">03. Equipment & Unit</label>
            <div className="flex gap-2"><div className="flex-1 flex gap-1 bg-slate-950 rounded-2xl p-1">{['E/L', 'E/S'].map(t => (<button key={t} onClick={() => setSelection({...selection, type: t, unit: '전체'})} className={`flex-1 py-2.5 rounded-xl text-[10px] font-black transition-all ${selection.type === t ? 'bg-white text-slate-950 shadow-md' : 'text-slate-600'}`}>{t}</button>))}</div><select value={selection.unit} onChange={(e) => setSelection({...selection, unit: e.target.value})} className="flex-[1.5] bg-slate-950 border-2 border-white/5 rounded-2xl px-3 text-[10px] font-black focus:border-indigo-600 outline-none text-slate-400">{availableUnits.map(u => <option key={u} value={u}>{u}</option>)}</select></div>
          </div>
        </section>

        {/* 선택 역사 점검 요약 - 필터와 규격 통일(py-4 px-6), 플로팅 제거 */}
        <section className="bg-slate-900/50 border border-white/5 py-4 px-6 rounded-[2.5rem] shadow-2xl flex items-center justify-between">
          <div className="flex items-center gap-3">
            <CheckCircle size={18} className="text-indigo-500" />
            <span className="text-[11px] font-black text-indigo-500 uppercase tracking-[0.2em] whitespace-nowrap">역사 요약</span>
          </div>
          <div className="flex gap-6 items-center">
            <div className="flex flex-col items-end">
              <span className="text-[9px] font-bold text-slate-600 uppercase tracking-widest">Elevator</span>
              <p className="text-xl font-black text-white">{stationStats.elCount}<span className="text-[10px] ml-1 opacity-20">건</span></p>
            </div>
            ...
            <div className="flex flex-col items-end">
              ...
              <p className="text-xl font-black text-white">{stationStats.esCount}<span className="text-[10px] ml-1 opacity-20">건</span></p>
            </div>
          </div>
        </section>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4 pb-20">
            {filteredResults.length > 0 ? filteredResults.map((item, idx) => (
              <div key={item.id} className={`bg-slate-900/40 p-8 rounded-[2.5rem] border transition-all ${idx === 0 ? 'border-indigo-500 shadow-2xl shadow-indigo-900/10' : 'border-white/5 hover:border-white/10'}`}>
                <div className="flex justify-between items-start mb-5 pb-5 border-b border-white/5">
                  ...
                  <div className="bg-slate-800 text-slate-400 px-3 py-1 rounded-md text-[9px] font-black tracking-widest border border-white/5 uppercase">CODE {item.category}</div>
                </div>
                <p className="font-bold text-slate-400 text-[16px] leading-[1.8] break-keep">{item.content}</p>
              </div>
            )) : (
              <div className="text-center py-20 bg-slate-900/20 rounded-[2.5rem] border border-dashed border-white/5 text-slate-600 font-bold">데이터가 없습니다.</div>
            )}
          </div>
        </div>
      </main>
